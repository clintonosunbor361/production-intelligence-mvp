...
export function CreateItemModal({ isOpen, onClose, onSuccess }) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setTicketData({ ticket_id: '', customer_name: '', notes: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        const invalidProduct = products.find(p => !p.product_type_id || p.quantity < 1);
        if (invalidProduct) {
            alert('Please select a product type and valid quantity for all items.');
            return;
        }

        setLoading(true);
        try {
            // Create each product formulation
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Log New Production Items" maxWidth="max-w-2xl">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <label className="block text-sm font-medium text-maison-secondary">Products *</label>
                    ...
                    <div className="space-y-3">
                        {products.map((prod, index) => (
                            <div key={index} className="flex items-start gap-4 p-3 bg-gray-50 rounded-lg border border-gray-100 relative">
                                <div className="flex-1">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => handleProductChange(index, 'product_type_id', e.target.value)}
                                    >
